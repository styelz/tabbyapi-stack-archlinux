...
def qwen_parser(**kwargs):
    ...
    return TagStreamParser(
        ...
        reasoning_end="</think>",
        tool_start=TOOLCALL_STARTS,
        tool_end=TOOLCALL_ENDS,
        **kwargs,
    )
...
class TagStreamParserTests(unittest.TestCase):
    ...
    def test_qwen_bare_function_tag_is_a_tool_start(self):
        # Qwen 3.6 often skips <tool_call> and emits <function=read> only
        ...
        self.assertEqual(out["content"], "I'll look.\n")
        self.assertIn("<function=read>", out["tool"])
        self.assertNotIn("<function=", out["content"])
        calls = parse_toolcalls(out["tool"])
        self.assertEqual(len(calls), 1)
        self.assertEqual(calls[0].function.name, "read")
        self.assertEqual(calls[0].function.arguments, '{"path": "/home/user/foo.py"}')

    def test_qwen_function_end_returns_to_content(self):
        from endpoints.OAI.utils.toolcall_formats.qwen3_coder import parse_toolcalls
        ...
        self.assertEqual(out["content"], "I'll look.\ndone looking.")
        self.assertNotIn("done looking", out["tool"])
        calls = parse_toolcalls(out["tool"])
        self.assertEqual(len(calls), 1)
        self.assertEqual(calls[0].function.name, "read")
        self.assertEqual(calls[0].function.arguments, '{"path": "/home/user/foo.py"}')

    def test_qwen_wrapped_call_stays_in_tool_until_tool_call_end(self):
        from endpoints.OAI.utils.toolcall_formats.qwen3_coder import parse_toolcalls
        ...
        self.assertEqual(out["content"], "I'll look.\ndone looking.")
        self.assertTrue(out["tool"].startswith("<tool_call>"))
        self.assertIn("</tool_call>", out["tool"])
        self.assertNotIn("done looking", out["tool"])
        calls = parse_toolcalls(out["tool"])
        self.assertEqual(len(calls), 1)
        self.assertEqual(calls[0].function.name, "read")

    def test_qwen_two_bare_functions_after_function_end(self):
        from endpoints.OAI.utils.toolcall_formats.qwen3_coder import parse_toolcalls
    ...
    def test_qwen_incomplete_function_still_parses(self):
        from endpoints.OAI.utils.toolcall_formats.qwen3_coder import parse_toolcalls
