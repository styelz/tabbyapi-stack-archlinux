...
class ClassifySkipTests(unittest.TestCase):
    ...
    def test_followup_questions_are_chat_not_comfy_prompts(self):
        ...
        from common.phrase_switch import looks_like_chat_not_image as chat_not_image

        self.assertFalse(
            chat_not_image("i want images created for the 6 panels, images images images")
        )
    ...
    def test_empty_nudge_does_not_replace_the_user_ask(self):
        from common.phrase_switch import last_user_text, refuses_new_images

        ask = (
            "there are 6 panels in the collection, there should be a unique "
            "image for each panel. the images should be generated by comfy"
        )
        nudge = (
        ...
        self.assertEqual(text, ask)
        self.assertFalse(refuses_new_images(text))
        blob = classify_blob(data)
...
class PlanTests(unittest.TestCase):
    ...
    def test_classify_system_build_uses_plan_assets(self):
        ...
        self.assertIn("generated by Comfy", CLASSIFY_SYSTEM)
...
class BuildPlanAssetRasterTests(unittest.TestCase):
    ...
    def test_comfy_panel_ask_is_explicit(self):
        data = _user(
            "there are 6 panels in the collection, there should be a unique "
            "image for each panel. the images should be generated by comfy"
        )
        ...
        self.assertTrue(
            _explicit_new_rasters(
                _user("i want images created for the 6 panels, images images images")
            )
        )
        self.assertFalse(
...
class ChatHoldTests(unittest.IsolatedAsyncioTestCase):
        ...
        data = ChatCompletionRequest(
            messages=[
                ChatCompletionMessage(
                    role="user",
                    content=(
                        "there are 6 panels in the collection, there should be "
                        "a unique image for each panel. the images should be "
                        "generated by comfy"
                    ),
                ),
                ChatCompletionMessage(
                    role="user",
                    content=(
                        "Continue. You stopped without changing files. Apply "
                        "the user's last request now with Write or StrReplace, "
                        "then give a short summary. Do not generate images "
                        "unless they asked."
                    ),
                ),
            ]
    ...
    async def test_reuse_after_comfy_ask_still_plans_new_dests(self):
        planned = [
            {
                "prompt": "qwen-image: sleeping castle",
                "output_path": "images/movies/sleeping-heart.png",
            }
        ]
        job = _job(status="queued")
        data = _user(
        ...
        with (
            ...
            mock.patch(
                "images.chat.classify_image_turn",
                new=mock.AsyncMock(return_value=ImageTurnPlan(action="reuse", items=[])),
            ),
            mock.patch(
                "images.chat.llm_plan_images",
                new=mock.AsyncMock(return_value=planned),
            ) as fallback,
            mock.patch(
                "images.chat._write_site_code",
                new=mock.AsyncMock(return_value=_write_code_response()),
            ),
            mock.patch(
                "images.chat.start_mcp_image_job",
                new=mock.AsyncMock(return_value=(job, "started")),
            ...
            mock.patch(
                "images.chat._first_code_pass_holds_llm", return_value=True
            ),
        ):
...
class LiveCodeStreamTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_dests_on_page_launch_without_another_code_pass(self):
        from images.chat import _write_page_then_maybe_launch
        ...
        data = _user(
            "there are 6 panels in the collection, there should be a unique image "
            "for each panel t suite the description and the style of the images "
            "should be in the websites style."
        )
        ...
        with (
            ...
            mock.patch("images.chat._missing_linked_page_files", return_value=[]),
            mock.patch("images.chat._missing_requested_pages", return_value=[]),
            mock.patch("images.chat._dests_already_on_page", return_value=True),
            mock.patch("images.chat._page_blocking_comfy", return_value=False),
            mock.patch("images.chat.note_coding_progress") as note,
        ):
            response, launch = await _write_page_then_maybe_launch(data, job, None)
    ...
    def test_keep_writing_does_not_block_unbound_dests(self):
        from images.chat import _keep_writing_page

        job = _job(id="abc-123", status="coding", owner="", chat_id="", code_turns=1)
        data = _user("there are 6 panels in the collection")
        read_only = ChatCompletionResponse(
            model="gpt-4o",
            choices=[
                ChatCompletionRespChoice(
        ...
        self.assertFalse(_keep_writing_page(read_only, job, data))

    async def test_dests_already_on_page_first_pass_starts_comfy(self):
        planned = [
            {"prompt": "qwen-image: panel one", "output_path": "images/panel1.png"}
        ]
        job = _job(id="panel-job", status="queued")
        data = _user(
            "there are 6 panels in the collection, there should be a unique image "
            "for each panel t suite the description"
        )
        with (
            mock.patch("images.chat.get_mcp_image_job", return_value=None),
            mock.patch("images.chat.active_mcp_image_job", return_value=None),
            ...
            mock.patch("images.chat._pages_on_disk", return_value=True),
            mock.patch("images.chat._dests_already_on_page", return_value=True),
            mock.patch("images.chat._profile_writes_files", return_value=True),
            mock.patch(
                "images.chat.start_mcp_image_job",
            ...
            mock.patch(
                "images.chat._hold_then_reply", new=mock.AsyncMock(return_value="held")
            ) as hold,
        ):
            result = await handle(
                data,
        ...
        self.assertTrue(start.await_args.kwargs.get("start", True))
        hold.assert_awaited()
