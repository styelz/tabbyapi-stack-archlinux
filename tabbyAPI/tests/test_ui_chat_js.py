...
class ChatJsStopQueueSteerTests(unittest.TestCase):
    ...
    def test_live_thought_formats_fences_without_rebuilding_trace(self):
        ...
        css_src = CHAT_CSS.read_text(encoding="utf-8")
        self.assertRegex(css_src, r"\.md-code pre \{[^}]*white-space: pre-wrap")
        self.assertRegex(css_src, r"\.md-code pre \{[^}]*overflow-wrap: anywhere")
        self.assertNotIn("max-height: min(28rem, 55vh)", css_src)
        self.assertNotIn("max-height: 280px", css_src)
    ...
    def test_code_git_pane(self):
        ...
        self.assertIn("if (!force && sig === gitPaintSig && filesGitList.childElementCount) {", self.src)
        self.assertIn("if (gitFocusMessage) focusGitMessage();", self.src)
        ...
        self.assertIn('id="chat-files-git-tools"', self.src)
        self.assertIn('gitToolButton("pull"', self.src)
        self.assertIn('gitToolButton("push"', self.src)
        self.assertIn('gitToolButton("sync"', self.src)
        self.assertIn('gitToolButton("fetch"', self.src)
        self.assertIn('gitToolButton("commit"', self.src)
        self.assertIn("function gitCommandItems(", self.src)
        self.assertIn("function gitFileMenuItems(", self.src)
        self.assertIn(': "Pull"', self.src)
        self.assertIn('label: "Fetch"', self.src)
        self.assertIn('label: "Commit"', self.src)
        self.assertIn('label: "Checkout to…"', self.src)
        self.assertIn("Ctrl+Enter to commit", self.src)
        self.assertIn("gitFileMenuItems(gitRow.dataset.path)", self.src)
        self.assertIn(".chat-git-toolbar", css)
        self.assertIn(".chat-git-tool", css)
