...
def resolve_template_vars(data: ChatCompletionRequest, container) -> dict:
    ...
    defaults = dict(container.template_vars_default)
    # reasoning: false only hid think tags from the parser. Qwen templates still
    # open <think> unless enable_thinking is false, so the think text leaked.
    if getattr(container, "reasoning", None) is False and "enable_thinking" not in defaults:
