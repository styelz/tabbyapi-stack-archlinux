...
**Update git** pulls. **Update all** also runs `install.sh --update` and restarts. Status can do the same. Auto-update (default every 7 days) does Update all when idle; `tsctl updates disable` or Settings → Updates turns it off. This does not overwrite `config.yml` or `tabby.env`, and it does not run `pacman -Syu`.
