...
CODE_SYSTEM = (
    ...
    "not dump whole files in chat. If you show a snippet, wrap it in a "
    "markdown fence with a language tag (```html, ```css, ```js). Do not "
    "paste HTML, CSS, or JS drafts into the assistant message. Do not try "
    "to run the site for the user; "
...
_FILE_WORK_PROMPT = re.compile(
    ...
    r"\b(?:undo|restore)\b|"
    r"\b(?:broken|isn'?t working|not working|nothing happens)\b|"
    r"\b(?:here is|this is|see) the problem\b|"
    r"\bthe problem with\b|"
    r"\b(?:hamburger|nav-toggle)\b|"
    r"\bduplicate\b|"
