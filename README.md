...
The menu defaults to **Simple** (this PC vs LAN, minimal coding and image models from Hugging Face, plus GPU-compatible optional models with disk-size estimates). The default is Qwen 9B, the CPU embedder, and Qwen-Image (~29 GiB of weights); 8 GB GPUs use Flux instead. Choose **Advanced** for encryption, full model control, bind address, public URL, reverse SSH, Omarchy, or to turn the screensaver off.
